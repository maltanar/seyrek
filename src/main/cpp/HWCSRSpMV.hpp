#ifndef HWCSRSPMV_HPP
#define HWCSRSPMV_HPP
...
public:
  ...
  virtual void setA(CSR<SpMVInd, SpMVVal> * A) {
    ...
    m_indPtrSize = sizeof(SpMVInd) * (m_A->getRows() + 1);
    ...
    set_csr_indPtr((AccelDblReg) m_acc_indPtrs);
    set_csr_cols(m_A->getCols());
    set_csr_inpVec((AccelDblReg) m_acc_x);
    set_csr_nz(m_A->getNNZ());
    set_csr_nzData((AccelDblReg) m_acc_nzData);
    set_csr_outVec((AccelDblReg) m_acc_y);
    set_csr_ind((AccelDblReg) m_acc_inds);
    set_csr_rows(m_A->getRows());
  }
...
protected:
  ...
  AccelReg readReg(HWCSRSpMVReg reg) {return m_platform->readReg(m_peNum * HWSPMVPE_REGS + reg);}
  void writeReg(HWCSRSpMVReg reg, AccelReg v) {m_platform->writeReg(m_peNum * HWSPMVPE_REGS + reg, v);}
  ...
  void set_csr_indPtr(AccelDblReg value) { writeReg(offsIndPtrHi, (AccelReg)(value >> 32)); writeReg(offsIndPtrLo, (AccelReg)(value & 0xffffffff)); }
  void set_csr_ind(AccelDblReg value) { writeReg(offsIndHi, (AccelReg)(value >> 32)); writeReg(offsIndLo, (AccelReg)(value & 0xffffffff)); }
  void set_csr_nzData(AccelDblReg value) { writeReg(offsNzDatHi, (AccelReg)(value >> 32)); writeReg(offsNzDatLo, (AccelReg)(value & 0xffffffff)); }
  void set_csr_inpVec(AccelDblReg value) { writeReg(offsInpVecHi, (AccelReg)(value >> 32)); writeReg(offsInpVecLo, (AccelReg)(value & 0xffffffff)); }
  void set_csr_outVec(AccelDblReg value) { writeReg(offsOutVecHi, (AccelReg)(value >> 32)); writeReg(offsOutVecLo, (AccelReg)(value & 0xffffffff)); }
  void set_csr_rows(AccelReg value) {writeReg(offsRows, value);}
  void set_csr_cols(AccelReg value) {writeReg(offsCols, value);}
  void set_csr_nz(AccelReg value) {writeReg(offsNZ, value);}
