...
// base class for doing SpMV-on-semirings for CSR-encoded sparse matrices
template <class SpMVInd, class SpMVVal>
class CSRSpMV : public virtual Semiring<SpMVInd, SpMVVal> {
public:
  CSRSpMV() {m_A = 0; m_x = 0; m_y = 0;}
  virtual ~CSRSpMV() {};

  virtual void setA(CSR<SpMVInd, SpMVVal> * A) {m_A = A;}
  CSR<SpMVInd, SpMVVal> * getA() {return m_A;}
